...
def noticias_info(event, context):
    date = datetime.datetime.now()
    year= date.year
    month= date.month
    day = date.day
    url = "headlines/raw/periodico=El tiempo/year="+str(year)+"/month="+str(month)+"/day="+str(day)+"/El tiempo.txt"
    url1 = "headlines/raw/periodico=El espectador/year="+str(year)+"/month="+str(month)+"/day="+str(day)+"/El espectador.txt"
    response1 = s3.get_object(Bucket="zappa-parcial2-descargas-noticias",Key=url)
    data1 = response1['Body'].read()
    print("hola", data1)
    response = s3.get_object(Bucket="zappa-parcial2-descargas-noticias",Key=url1)
    ...
    s3.upload_file("/tmp/tiempo.csv","zappa-parcial2",url11)
    url12 = "Headlines/news/periodico=El Espectador""/year="+str(year)+"/month="+str(month)+"/day="+str(day)+"/espectador.csv"
    s3.upload_file("/tmp/espectador.csv","zappa-parcial2",url12)

def lambda_handler(event, context):
    bucket_name = 'zappa-parcial2'
    client = boto3.client('athena')
    config = {
        'OutputLocation': 's3://zappa-parcial2/Headlines/news/periodico=El Espectador/year=2021/month=10/',
        'EncryptionConfiguration': {'EncryptionOption': 'SSE_S3'}
    }
    # Query Execution Parameters
    sql = 'MSCK REPAIR TABLE newsespectador'
    context = {'Database': 'info_news'}

    client.start_query_execution(QueryString = sql, QueryExecutionContext = context,ResultConfiguration = config)

    config = {
        'OutputLocation': 's3://zappa-parcial2/Headlines/news/periodico=El Tiempo/year=2021/month=10/',
        'EncryptionConfiguration': {'EncryptionOption': 'SSE_S3'}
    }
    # Query Execution Parameters
    sql = 'MSCK REPAIR TABLE newstiempo'
